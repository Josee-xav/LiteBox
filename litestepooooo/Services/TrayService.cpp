...
void TrayService::initTrayService()
{
	...
	for (int i = 0; i < trayItems.size(); i++) {
		...
		YLogger::Debug("LINE 52, appending tray btn via send message mthod. tray name:  %S", trayItems.at(i)->sIconText);
	}
	...
	if (hinstTrayLib != NULL)
	{
		...
		YLogger::Error("Couldnt load dll .");
	}




	LB_Api::register_class(trayClassName.c_str(), LB_Api::main_hinstance, trayProc, 0);
...
void TrayService::trayEvent(void* data, unsigned size)
{
	...
	case NIM_ADD:
	{
		...
		for (int i = 0; i < trayBtnList->m_Items.size(); i++) {
			...
			if (ni->hWnd == nid.hWnd) {
				YLogger::Debug("found a already existing tray icon, trying to be added again..   : %S", ni->sIconText);
				return;
		...
		tray->uID = nid.uID;
		YLogger::Debug("adding button   : %S", tray->sIconText);
	...
	case NIM_DELETE:
	{
		for (int i = 0; i < trayBtnList->m_Items.size(); i++) {
			...
			TrayEntryBtn::TrayItem* ni = (TrayEntryBtn::_TrayItem*)item->m_data;
			if (ni->hWnd == nid.hWnd) {
				YLogger::Debug("removing a button..   : %S", ni->sIconText);
